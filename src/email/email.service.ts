...
export class EmailService {
  ...
  async sendNewApplicationNotification(application: {
    ...
    try {
      ...
      this.logger.log(
        `Уведомление о новой заявке отправлено на ${process.env.MAIN_COMPANY_EMAIL}`,
      );
    } catch (error) {
  ...
  async sendApplicationUpdateNotification(
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      this.logger.log(
        `Уведомление отправлено владельцу заявки (${recipientEmail}).`,
      );
    } catch (error) {
      this.logger.error(
        'Ошибка при отправке email-уведомления владельцу заявки',
        error,
      );
      throw error;
    }

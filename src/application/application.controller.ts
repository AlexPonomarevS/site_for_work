...
export class ApplicationController {
  ...
  async exportApplicationToExcel(
    ...
    res.set(
      'Content-Disposition',
      `attachment; filename=application_${applicationId}.xlsx`,
    );
    res.type(
  ...
  async createNote(
    ...
    return this.applicationService.createNote(
      +applicationId,
      body.content,
      createdBy,
    );
  ...
  async updateStatus(
    ...
    try {
      const updated = await this.applicationService.updateStatus(
      ...
      return res
        .status(500)
        .json({ error: 'Не удалось обновить статус заявки' });
    }
  }

  @Put(':applicationId/section')
  async updateSection(
    @Param('applicationId') applicationId: string,
    ...
    try {
      const updated = await this.applicationService.updateSection(
        parseInt(applicationId),
        body.section,
      );
      return res.status(200).json(updated);
